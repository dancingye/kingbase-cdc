...
public final class InstantiationUtil {
    ...
    public static class ClassLoaderObjectInputStream extends ObjectInputStream {
        ...
        private static final ClassLoader springClassLoader = Thread.currentThread().getContextClassLoader();

        public ClassLoaderObjectInputStream(InputStream in, ClassLoader classLoader)
                throws IOException {
            super(in);
            //在这里把类加载器换成SpringBoot的类加载器, 否则打包成jar包之后运行flink任务会报错找不到flink的依赖
            //https://wenku.csdn.net/answer/54qs372gvkuv
